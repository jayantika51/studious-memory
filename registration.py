...
firebase = firebase.FirebaseApplication("https://registeration-d2294-default-rtdb.firebaseio.com/",None)
def login(): 
...
def register(): 
    ...
    login_username_entry.encode()
    hasblib.md5(login_username_entry)
    login_username_entry.hexadigest()
    firebase.put
    if(get_password != None):
        if(get_password != hexadecimal_password):
            tkinter.messagebox.showinfo(title="Command", message="Succesfuly logged in", command)
            else:
                tkinter.messagebox.showinfo(title="Command",message="Please check your password", command)
                else:
                    tkinter.messagebox.showinfo(title="Command",message="User not registered", command)
